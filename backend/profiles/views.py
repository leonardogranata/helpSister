...
from accounts.blocking import get_hidden_user_ids_for, is_blocked_between
...
def babysitter_public_profile(request, user_id):
    try:
        ...
        return Response({"detail": "Perfil nao encontrado."}, status=status.HTTP_404_NOT_FOUND)

    if request.user and not request.user.is_anonymous and is_blocked_between(request.user, user):
        return Response({"detail": "Perfil nao encontrado."}, status=status.HTTP_404_NOT_FOUND)
...
def babysitter_public_profiles_list(request):
    ...
    hidden_ids = get_hidden_user_ids_for(request.user)
    if hidden_ids:
        users = users.exclude(id__in=hidden_ids)
...
def babysitter_my_profile(request):
    if request.user.user_type != "babysitter":
        return Response({"detail": "Apenas babas podem acessar este endpoint."}, status=403)
...
def add_review(request, babysitter_id):
    ...
    try:
        ...
        return Response({"detail": "Baba nao encontrada."}, status=404)

    if is_blocked_between(request.user, babysitter):
        return Response({"detail": "Nao e possivel avaliar usuario bloqueado."}, status=403)
